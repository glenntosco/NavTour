  ...
  try {
    ...
    for (const sheet of Array.from(document.styleSheets)) {
      try {
        ...
        // Cross-origin — skip (CORS bypass may help on retry)
      }
    }
    ...
    origAll.forEach((origEl, i) => {
      ...
      Array.from(shadow.children).forEach(child => wrapper.appendChild(child.cloneNode(true)));
      ...
      cloneAll[i].appendChild(wrapper);
    });
    ...
    clone.querySelectorAll("[srcset]").forEach(el => {
      ...
      if (ss) el.setAttribute("srcset", ss.replace(/(\S+)(\s+\S+)?/g, (_: string, url: string, desc: string) => absUrl(url, baseUri) + (desc || "")));
    });
