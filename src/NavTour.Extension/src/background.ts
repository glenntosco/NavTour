...
async function captureTab(tabId: number, state: CaptureState) {
  try {
    ...
    // Send capture request to content script (capture.ts) via messaging
    // This supports async operations (font fetching, CORS stylesheet loading)
    const result = await new Promise<{ html: string; title: string; url: string } | null>((resolve) => {
      ...
      chrome.tabs.sendMessage(tabId, { type: "navtour:capture" }, (response) => {
        ...
        if (chrome.runtime.lastError) {
          console.warn("[NavTour] Content script not ready:", chrome.runtime.lastError.message);
          resolve(null);
        } else {
          resolve(response);
        }
...
// --- DOM capture is now handled by capture.ts content script ---
// Triggered via chrome.tabs.sendMessage({ type: "navtour:capture" })

// Legacy capturePageDOMAsync removed — capture logic is now in capture.ts
// which runs as a persistent content script and supports async operations.
